...
const filepath = path.resolve('e:/brkthru-digital-landing/index.html');
...
try {
    ...
    const targetText = 'What Leaders Say About the Book';
    const foundIdx = lines.findIndex(line => line.includes(targetText));

    if (foundIdx === -1) {
        console.error(`Target text "${targetText}" not found.`);
        // Print a few lines for context
        console.log("Lines 415-425 context:");
        for (let i = 414; i < Math.min(lines.length, 425); i++) {
            console.log(`${i+1}: ${JSON.stringify(lines[i])}`);
        }
        process.exit(1);
    }

    console.log(`Found target on line ${foundIdx + 1}`);

    const newContent = `              <p class="text-[0.6rem] font-black text-amber-500 text-center uppercase tracking-[0.4em] mb-8">Featured Experience & World Class Coach Book Testimonials</p>
