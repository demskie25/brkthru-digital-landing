...
updated_lines = []
target_found = False
target_str = "What Leaders Say About the Book"

for i, line in enumerate(lines):
    updated_lines.append(line)
    if target_str in line:
        print(f"Found target string at line {i+1}")
        if not target_found:
...
if target_found:
    for i in range(len(updated_lines)):
        if target_str in updated_lines[i]:
            updated_lines[i] = updated_lines[i].replace(target_str, 'Featured Experience & World Class Coach Book Testimonials')
    ...
    print(f"Target string '{target_str}' NOT found in index.html")
    # Let's try matching a partial string or checking case
    for i, line in enumerate(lines):
        if "Leaders Say" in line:
            print(f"Found similar string: '{line.strip()}' at line {i+1}")
    sys.exit(1)
