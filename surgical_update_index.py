...
try:
    ...
    # Target line 420 (0-indexed 419)
    target_idx = 419
    if 'What Leaders Say About the Book' in lines[target_idx]:
        print(f"Found target on line {target_idx + 1}")
        ...
        # Replace the single line with many lines
        lines[target_idx] = "".join(new_lines)
        
        with open(filepath, 'w', encoding='utf-8') as f:
            f.writelines(lines)
        print("Successfully updated index.html")
    else:
        print(f"Error: Target text not found on line {target_idx + 1}")
        print(f"Content found: {repr(lines[target_idx])}")
        # Search for it
        for i, line in enumerate(lines):
            if 'What Leaders Say About the Book' in line:
