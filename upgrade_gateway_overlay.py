...
premium_html = """
        <div x-show="!enneagramUnlocked" class="fixed inset-0 z-[9999] flex items-center justify-center overflow-hidden">
            <!-- Premium Background -->
            ...
            <div class="glass-card max-w-md w-full rounded-[3rem] p-10 lg:p-14 relative z-10 bg-white/95 backdrop-blur-2xl border border-white/30 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)]">
                ...
                <div class="space-y-6">
                    <a href="https://hitpay.app/pay/brkthru" class="block w-full py-5 rounded-2xl text-white font-black text-lg uppercase tracking-wider text-center bg-blue-700 shadow-xl hover:bg-blue-800 transition-all">
                        Unlock Access Now
                    </a>
...
# Regex to find the x-show="!enneagramUnlocked" div block
pattern = re.compile(r'<div x-show="!enneagramUnlocked".*?</div>\s*</div>', re.DOTALL)
...
for fpath in files:
    ...
    try:
        ...
        # We need a more robust way to find the closing div of the overlay
        # Since the overlay has a nested div, we look for the specific pattern of the old "Access Restricted" gate
        old_gate_pattern = re.compile(r'<div x-show="!enneagramUnlocked".*?Access Restricted.*?</a>\s*</div>\s*</div>', re.DOTALL)
        
        if old_gate_pattern.search(content):
            new_content = old_gate_pattern.sub(premium_html, content)
            with open(fpath, 'w', encoding='utf-8') as f:
                f.write(new_content)
            print(f"Successfully updated {fpath}")
        else:
            print(f"Could not find matching gate in {fpath}")
