...
new_content = """<p class="text-[0.6rem] font-black text-amber-500 text-center uppercase tracking-[0.4em] mb-8">Featured Experience & World Class Coach Book Testimonials</p>
...
# Match the title line with flexible whitespace
pattern = r'<p class="text-\[0\.6rem\] font-black text-amber-500 text-center uppercase tracking-\[0\.4em\] mb-8">What Leaders Say About the Book</p>'

if re.search(pattern, content):
    new_content_full = re.sub(pattern, new_content, content)
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(new_content_full)
    print("Successfully updated index.html using regex")
else:
    print("Pattern NOT found using default regex. Trying case-insensitive and flexible whitespace...")
    pattern_flex = r'<p[^>]*class="[^"]*text-\[0\.6rem\][^"]*amber-500[^"]*"[^>]*>\s*What Leaders Say About the Book\s*</p>'
    match = re.search(pattern_flex, content, re.IGNORECASE)
    if match:
        print(f"Flexible pattern found: '{match.group(0)}'")
        new_content_full = re.sub(pattern_flex, new_content, content, flags=re.IGNORECASE)
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(new_content_full)
        print("Successfully updated index.html using flexible regex")
    else:
        print("Flexible pattern also NOT found. Printing snippet for debugging:")
        # Find anything related to 'Leaders' or 'Book' in the vicinity
        start_pos = content.find('Pillar 1 Testimonials')
        if start_pos != -1:
            print(content[start_pos:start_pos+300])
        sys.exit(1)
